...
export class Renderer {
  ...
  draw(grid, hoverCell) {
    ...
    const results = traceAll(grid);
    const lit = results.lit.length > 0;
    ...
    for (let y = 0; y < GRID_SIZE; y++) {
      for (let x = 0; x < GRID_SIZE; x++) {
        ...
        if (cell === CELL.WALL) {
          ...
          const hit = results.lit.some(r => r.x === x && r.y === y);
          ctx.strokeStyle = hit ? COLORS.SUN : '#554400';
          ctx.lineWidth = hit ? 2.5 : 1.5;
          ctx.shadowColor = hit ? COLORS.SUN : 'transparent';
          ctx.shadowBlur = hit ? 10 : 0;
          ctx.strokeRect(px + 4, py + 4, this.cellSize - 8, this.cellSize - 8);
          ctx.shadowBlur = 0;
          ctx.fillStyle = hit ? COLORS.SUN : '#332200';
